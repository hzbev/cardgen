            ...
            {index == "3" &&
                <>
                    ...
                    {Object.keys(customText).map((x, i) => (
                        ...
                        className={`w-max h-max text-center target-${x}`} style={{ zIndex: x == activeText ? 20 : 10, fontSize: `${customText[x].size}px`, color: customText[x].color.includes("gradient") ? "transparent" : customText[x].color, backgroundImage: customText[x].color.includes("gradient") ? customText[x].color : "none", backgroundClip: customText[x].color.includes("gradient") ? "text" : "none"  }}>
                            {customText[x].text}
                        </div>
                    ))}
